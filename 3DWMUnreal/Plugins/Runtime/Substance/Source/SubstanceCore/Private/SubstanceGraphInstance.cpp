...
void USubstanceGraphInstance::PostDuplicate(bool bDuplicateForPIE)
{
	...
	if (!bDuplicateForPIE)
	{
		...
		{
			if (It->ParentFactory == ParentFactory && It->PackageURL == PackageURL && It->Instance)
			{
				RefInstance = (*It);
				break;
			}
		...
		if (GIsEditor)
		{
			...
			if (RefInstance->CreatedMaterial != NULL)
			{
				...
				CurrentFullPath.Split(TEXT("/"), &(MaterialPath), NULL, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
				...
				UObject* MaterialBasePackage = CreatePackage(NULL, *FullAssetPath);
...
bool USubstanceGraphInstance::SetInputImg(const FString& Name, class UObject* Value)
{
	if (Instance)
	{
		if (Substance::Helpers::IsSupportedImageInput(Value))
		{
			...
			Substance::Helpers::UpdateInput(this->Instance, nullptr, Value);
