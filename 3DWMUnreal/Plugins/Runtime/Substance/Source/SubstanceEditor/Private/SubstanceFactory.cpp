...
struct ReimportData
{
	...
	SubstanceAir::Preset PreviousPresetData;

	ReimportData(FString P, UMaterial* M, SubstanceAir::Preset SAP, TArray<MaterialParameterSet>& RM)
	{
		PackageURL = P;
...
void USubstanceFactory::RecreateGraphsPostReimport(USubstanceInstanceFactory* ParentFactory, UObject* InParent)
{
	...
	for (; GrItr; ++GrItr)
	{
		...
		//Apply presets
		(*GrItr).PreviousPresetData.apply(*NewInstance->Instance);
...
{
	//Clear the map of data to not attempt to recreate instances of a previous reimported factory
	...
	for (; GraphItr; ++GraphItr)
	{
		...
		SubstanceAir::Preset StorePreset;
		StorePreset.fill(*(*GraphItr)->Instance);

		//Set all of the reference data needed for this graph
		PreviousInstanceTransfer.Add(ReimportData((*GraphItr)->PackageURL, (*GraphItr)->CreatedMaterial, StorePreset, ReferencingMaterials));
	}
}
